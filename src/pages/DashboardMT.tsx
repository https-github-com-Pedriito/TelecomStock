  ...
  return (
    <div className="w-full max-w-full">
      <div className="space-y-6">
        {/* Header */}
        <div className="mb-6">
          <Typography variant="h3" className="mb-2 text-xl md:text-3xl text-gray-900 dark:text-white">
            Tableau de bord
          </Typography>
          <Typography variant="lead" className="text-sm md:text-base text-gray-600 dark:text-gray-400">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6 mb-6">
          {statsCards.map((card, index) => (
            <Card key={index} className="shadow-sm border border-gray-200 dark:border-gray-700 w-full overflow-hidden bg-white dark:bg-gray-800">
              <CardBody className="p-4 md:p-6">
                <div className="flex flex-col space-y-2">
                  <div className="flex items-center justify-between">
                    <Typography variant="small" className="font-normal text-gray-600 dark:text-gray-400 text-xs md:text-sm truncate">
                      {card.title}
                    </Typography>
                    <div className={`p-2 rounded-lg bg-${card.color}-50 dark:bg-${card.color}-900/30 flex-shrink-0`}>
                      <card.icon className={`w-5 h-5 md:w-6 md:h-6 text-${card.color}-500 dark:text-${card.color}-400`} />
                    </div>
                  </div>
                  <Typography variant="h4" className="text-xl md:text-2xl font-bold text-gray-900 dark:text-white">
                    {card.value}
                  </Typography>
                </div>
              </CardBody>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 md:gap-6 mb-6">
          ...
          <CardHeader floated={false} shadow={false} className="rounded-none bg-white dark:bg-gray-800">
          ...
          <CardBody className="px-6 pb-6">
            {articlesWithAlerts.length === 0 ? (
              <Typography variant="small" className="text-center py-8 text-gray-500 dark:text-gray-400">
                Aucune alerte stock
              </Typography>
            ) : (
          ...
          {recentMouvements.length === 0 ? (
            ...
            <div className="overflow-x-auto">
              <table className="w-full min-w-max table-auto text-left">
                <thead>
                  <tr>
                    <th className="border-b border-gray-200 dark:border-gray-700 py-3 px-6">
                      <Typography variant="small" className="font-normal leading-none text-gray-500 dark:text-gray-400">
                        Date
                      </Typography>
                    </th>
                    <th className="border-b border-gray-200 dark:border-gray-700 py-3 px-6">
                ...
                <tbody>
                  {recentMouvements.map((mouvement, index) => {
                    ...
                    const classes = isLast ? "py-3 px-6" : "py-3 px-6 border-b border-gray-200 dark:border-gray-700";
